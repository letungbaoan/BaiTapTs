...
class MemoryGame {
    constructor() {
        ...
        this.bestKey = "memory_best_score";
    ...
    startGame() {
        ...
        this.render();
        if (pairs === 8)
            this.boardEl.className = "grid grid-cols-4 gap-8";
        else if (pairs === 18)
            this.boardEl.className = "grid grid-cols-6 gap-8";
        else
            this.boardEl.className = "grid grid-cols-8 gap-8";
        this.startTimer();
    }
    createCards(pairs, theme) {
    ...
    syncDomForCard(card) {
        const el = this.boardEl.querySelector(`[data-id='${card.id}']`);
        if (el)
